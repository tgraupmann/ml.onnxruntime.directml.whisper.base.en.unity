...
This is a Windows x64 package that embeds the OpenAI Whisper base English model inside a single native DLL, accelerated by ONNX Runtime with DirectML. Supports the Unity Editor and Windows Standalone 64-bit builds. Setup is turnkey and fully offline: no downloads, no updates—unpack the DLL, open the example scenes directly from the package, and choose your audio source (clip or microphone) for transcription or dictation.
...
## What’s inside
- Native plugin built on **ONNX Runtime + DirectML**; the full Whisper base EN model is contained in one DLL (no external model downloads, works fully offline).
...
## Package contents
- **Plugins/Windows/x64**: native DirectML/ONNXRuntime DLL with the embedded Whisper base EN model (unpacked via menu).
...
## Quick setup
1) **Unpack the DLL:** `Assets → WhisperBaseEn → Unpack WhisperBaseEn DLLs` (combines the split archive into a single native DLL in `Plugins/Windows/x64`). The native DLL is shipped split so each file stays under common version-control size limits (often ~100 MB). If your repo enforces that limit, you can add the unpacked DLL to `.gitignore` and rerun **Unpack WhisperBaseEn DLLs** after pulling or on machines that need to rebuild the extracted binary.
2) **Unpack test audio:** `Assets → WhisperBaseEn → Unpack WhisperBaseEn Example Audio` also drops the sample `WhisperTest.wav` into `Assets/StreamingAssets/Audio` for the clip-based example.
...
## Minimal API
```csharp
var whisperBaseEn = new DllWhisperBaseEn();          // create the client (loads the native DLL/model)

// pcm: float[] mono, 16 kHz samples
var language = whisperBaseEn.DetectLanguage(pcm);    // detect language
var transcript = whisperBaseEn.Decode(pcm);          // decode transcript
```
...
### Progress
- `DllWhisperBaseEn.SetProgress(DllWhisperBaseEn.ProgressCallback callback)`
...
- `DllWhisperBaseEn.ClearProgress()`
...
### Cancellation
- `DllWhisperBaseEn.Abort()`
	- Requests cooperative cancellation of the currently-running native operation (for example, an in-flight `Decode`).
	- This is a global/native cancellation flag; it is not tied to a specific `DllWhisperBaseEn` instance.
